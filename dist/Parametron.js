...
const chinchilla_1 = __importDefault(require("chinchilla"));
...
exports.parametronV2 = parametronV2;
...
class Parametron {
    ...
    fire() {
        ...
        this.pact = new bluebird_1.default((resolve, reject) => {
            ...
            const body = chinchilla_1.default.new('viscacha', 'request', {
                request: {
                    model: this.opts.model,
                    opts: { rawRequest: true },
                    action: this.opts.action || '$c.search',
                    params: this.opts.params,
                    body: lodash_1.merge({}, this.opts.params, {
                        search: { filters },
            ...
            chinchilla_1.default(body).$c('get', {}, { rawRequest: true, rawResult: true }).then((result) => {
                ...
                const aggregations = lodash_1.reduce(result.aggregations, (aggs, result, key) => {
                    let values;
                    key = lodash_1.replace(key, 'count_by_', '');
                    values = lodash_1.map(result.buckets, (v) => ({ value: v.key, count: v.doc_count }));
                    return lodash_1.extend(aggs, { [key]: values });
                }, {});
